...
def tsne_plot_all(objects: list, perplexity: int = 5,
    ...
    tsne_results = tsne.fit_transform(np.vstack([objects[0], objects[1], objects[2], objects[3]]))
    tsne_user_protos = tsne_results[:len(objects[0])]
    tsne_user_emb = tsne_results[len(objects[0]):len(objects[0])+len(objects[1])]
    tsne_item_protos = tsne_results[len(objects[0])+len(objects[1]):len(objects[0]) + len(objects[1])+len(objects[2])]
    tsne_item_emb = tsne_results[len(objects[0]) + len(objects[1])+len(objects[2]):len(objects[0]) + len(objects[1])+len(objects[2])+len(objects[3])]

    plt.figure(figsize=(6, 6), dpi=100)
    plt.scatter(tsne_user_emb[:, 0], tsne_user_emb[:, 1], s=10, alpha=0.6, c='#FFA54F', label='user_emb')
    plt.scatter(tsne_item_emb[:, 0], tsne_item_emb[:, 1], s=10, alpha=0.6, c='#EEEE00', label='item_emb')

    plt.scatter(tsne_item_protos[:, 0], tsne_item_protos[:, 1], s=30, alpha=0.6, c='#C1CDCD', label='item_protos')
    plt.scatter(tsne_user_protos[:, 0], tsne_user_protos[:, 1], s=30, c='#d73027', alpha=0.9, label='user_protos')
