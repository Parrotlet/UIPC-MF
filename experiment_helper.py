...
def start_hyper(conf: dict, model: str, dataset: str, seed: int = SINGLE_SEED):
    ...
    if dataset == 'lfm2b_2020' or dataset == 'ml-1m' or dataset == 'm4_3m':
    ...
    tune.register_trainable(group_name, start_training) # training model
    analysis = tune.run(
        group_name,
        config=conf,
        ...
        callbacks=[callback],
        metric='_metric/' + 'val_uipc_mf_l1_'+OPTIMIZING_METRIC,
        mode='max'
    )
    metric_name = '_metric/' + 'val_uipc_mf_l1_' + OPTIMIZING_METRIC
